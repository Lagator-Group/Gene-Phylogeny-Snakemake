...
def build_distance_matrix(tree, taxa):
    ...
    name2node = {leaf.name: leaf for leaf in tree.iter_leaves()}
    ...
    for i, t1 in enumerate(taxa):
        ...
        for j, t2 in enumerate(taxa):
            ...
            d = tree.get_distance(name2node[t1], name2node[t2])
...
def cophenetic_scatter(tree1_path, tree2_path, save_path):
    ...
    shared = sorted(set(tree1.get_leaf_names()) & set(tree2.get_leaf_names()))
    ...
    plt.scatter(vals1, vals2, s=5, alpha=0.3)
    ...
    plt.title("Cophenetic Distance Comparison\n" f"Pearson r = {corr:.3f}")
...
try:
    ...
    print(e)
    fig, ax = plt.subplots()
    ax.text(0.5, 0.5, "No data / plot failed", ha="center", va="center")
    ax.set_axis_off()
