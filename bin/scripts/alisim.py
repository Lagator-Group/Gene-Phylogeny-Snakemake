...
# load best model from iqtree
iqtree_path = Path(gene1_iqtree)

# if iqtree_path.exists():
#     print(iqtree_path.read_text()[:1000])
...
# rebuild trees from alignments
for file in glob.glob(f"{tmp_dir}/*.phy"):
    # print(file)
    # print(cmd)
    cmd = f"iqtree3 -s {tmp_dir}/{file} -st AA -ntmax {snakemake.threads} --prefix {tmp_dir}/{gene2} -m {model}"
    subprocess.run(cmd, shell=True, capture_output=True, text=True)

#### Mantel Exec and Save
print('Mantel')
try:
    mantel_df = mantel_calc(gene1_tree, gene2_tree)
    mantel_obs = mantel_df["Mantel r"]

    null_list = []
    for file in glob.glob(f"{tmp_dir}/{gene2}*.treefile"):
        print(file)
        mantel_null_df = mantel_calc(f"{file}", gene1_tree)
        mantel_null_r = mantel_null_df["Mantel r"]
        null_list.append(mantel_null_r)
    if len(null_list) == 0:
        mantel_sim_p = np.nan
    else:
        null_array = np.array(null_list)
        mantel_sim_p = (np.sum(null_array >= mantel_obs) + 1) / (len(null_array) + 1)
        
except Exception as e:
    print(f'Mantel failed for {gene1} and {gene2}: {e}. Skipping mantel calculation')
    mantel_df = pd.DataFrame(
        columns=["Gene 1", "Gene 2", "Mantel r", "p", "n", "Mantel Sim p"]
    )
    mantel_df.loc[0] = [gene1, gene2, np.nan, np.nan, np.nan, np.nan]
    mantel_sim_p = np.nan


#### Robinson-Foulds Exec and Save
print('Robinson-Foulds')
try:
    rf_df = robinson_fould(gene1_tree, gene2_tree)
    nrf_obs = rf_df["Normalized RF"]

    null_list = []
    for file in glob.glob(f"{tmp_dir}/{gene2}*.treefile"):
        print(file)
        nrf_null_df = robinson_fould(f"{file}", gene1_tree)
        nrf_null_r = nrf_null_df["Normlized RF"]
        null_list.append(nrf_null_r)
    if len(null_list) == 0:
        nrf_sim_p = np.nan
    else:
        null_array = np.array(null_list)
        nrf_sim_p = (np.sum(null_array >= nrf_obs) + 1) / (len(null_array) + 1)

except Exception as e:
    print(f'Robinson-Foulds failed for {gene1} and {gene2}: {e}. Skipping Robinson-Foulds calculation')
    rf_df = pd.DataFrame(
        columns=[
            "Gene 1",
            "Gene 2",
    ...
    rf_df.loc[0] = [gene1, gene2, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]
    nrf_sim_p = np.nan
